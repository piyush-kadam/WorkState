    ...
    <footer className="relative bg-black text-white overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <p className="text-white/70 text-base md:text-lg max-w-md">
             Your go-to digital solution. Where innovation meets limitless possibilities. 
                We craft digital experiences that transcend expectations.
            </p>
            ...
            <div className="space-y-3">
              {services.map((s, i) => (
                <div key={i} className="flex items-center space-x-2 h-10">
                  ...
                  <span className="text-white/70 hover:text-white transition-all">{s}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-3">
              {company.map((c, i) => (
                <a key={i} href={c.href} className="flex items-center space-x-2 text-white/70 hover:text-white transition-all h-10">
                  <div className="w-2 h-2 bg-white/50 rounded-full"></div>
                  <span>{c.name}</span>
                </a>
              ))}
        ...
        <div className="mt-12 pt-6 border-t border-white/10 flex flex-col md:flex-row justify-between items-center text-center md:text-left space-y-3 md:space-y-0">
          <div>
            <p className="text-white/60 text-base md:text-lg">© {currentYear} WorkState. All rights reserved.</p>
            <p className="text-white/50 text-sm md:text-base">
              {currentTime.toLocaleDateString()} | {currentTime.toLocaleTimeString()}
            </p>
          </div>
          <p className="text-white/50 text-base md:text-lg italic">
            Designed & Developed by <span className="font-semibold text-white hover:text-blue-400 cursor-pointer">PiyushKadam</span>
          </p>
        </div>
      </div>
    </footer>
...
function SocialIcon({ href, children }) {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" className="group relative w-12 h-12 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full flex items-center justify-center hover:bg-white/20 hover:scale-110 transition-all duration-300 overflow-hidden">
