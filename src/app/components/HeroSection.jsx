  ...
  return (
    <div className="relative">
      ...
      <section className="relative min-h-screen overflow-hidden bg-black">
        ...
        <div className={`absolute inset-0 transition-all duration-2000 ease-out ${animationStep >= 1 ? 'opacity-100 scale-100' : 'opacity-0 scale-105'}`}>
          <img src="/h1.jpeg" alt="Where innovation meets imagination" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-b from-black/60 via-transparent to-black/40"></div>
        </div>

        {/* Text Overlay */}
        <div className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-1500 delay-700 ${animationStep >= 2 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`}>
          <div className="text-center px-8 py-12 rounded-3xl bg-black/20 backdrop-blur-md border border-white/10 max-w-4xl mx-auto">
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-white leading-tight mb-6">
              ...
              <span className="block text-transparent bg-clip-text bg-gradient-to-r from-white via-gray-200 to-white animate-pulse"> DIGITAL SOLUTION </span>
            </h1>
            <div className="h-px w-32 bg-gradient-to-r from-transparent via-white to-transparent mx-auto mb-8 opacity-60"></div>
            <p className="text-lg md:text-xl text-white/80 max-w-2xl mx-auto leading-relaxed">Where innovation meets limitless possibilities. We craft digital experiences that transcend expectations.</p>
          </div>
        </div>

        {/* CTA */}
        <div className={`absolute bottom-0 left-0 right-0 pb-20 transition-all duration-1000 delay-1200 ${animationStep >= 3 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          {particlePositions.slice(0, 8).map((particle, i) => (
            <div key={i} className={`absolute bg-white/10 rounded-full transition-all duration-3000 ${animationStep >= 2 ? 'opacity-100' : 'opacity-0'}`}
              style={{
          ...
          {particlePositions.slice(8).map((particle, i) => (
            <div key={`accent-${i}`} className={`absolute w-1 h-1 bg-white/20 rounded-full transition-all duration-4000 ${animationStep >= 3 ? 'opacity-100' : 'opacity-0'}`}
              style={{
                left: particle.left,
                top: particle.top,
                animationDelay: `${particle.delay}ms`,
        ...
        {/* Loading Overlay */}
        <div className={`absolute inset-0 bg-black transition-opacity duration-1000 ${isLoaded ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
      ...
      {/* Video Section */}
      <section className="relative min-h-screen bg-black border-t border-white/10 overflow-hidden flex items-center">
        <div className="container mx-auto px-8 py-20">
          <div className="grid grid-cols-1 lg:grid-cols-7 gap-12 items-center">
            {/* Left Words */}
            <div className="lg:col-span-2 relative h-96 flex items-center justify-start">
              <div className="text-left space-y-8">
                ...
                <div className="overflow-hidden h-[70px]">
                  <span key={leftWordIndex} className="block text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-gray-300 to-white animate-slideIn">
            ...
            <div className="lg:col-span-3 relative flex justify-center">
              <div className="relative w-full max-w-5xl">
                <div className="relative overflow-hidden rounded-2xl">
                  <video autoPlay loop muted playsInline className="w-full h-[700px] object-cover bg-black">
            ...
            {/* Right Words */}
            <div className="lg:col-span-2 relative h-96 flex items-center justify-end">
              <div className="text-right space-y-8">
                <div className="text-5xl md:text-6xl font-black text-white leading-tight">
                  <div className="text-white/60">freelancing</div>
                  <div>brings</div>
                </div>
                <div className="overflow-hidden h-[70px]">
                  <span key={rightWordIndex} className="block text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-l from-white via-gray-300 to-white animate-slideIn">
            ...
            <div className="lg:col-span-2 space-y-8">
              <div className="space-y-4">
                <div className="inline-block">
                  <span className="px-4 py-2 bg-white/10 backdrop-blur-md border border-white/20 text-white/80 text-sm font-medium rounded-full"> WHAT WE CREATE </span>
                </div>
                <h2 className="text-4xl md:text-6xl font-black text-white leading-tight">
                  ENDLESS <br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-gray-300 to-white"> POSSIBILITIES </span>
                </h2>
              ...
              <div className="space-y-6">
                {[
                  ...
                  <div key={index} className="group flex items-start space-x-4 p-4 rounded-xl hover:bg-white/5 transition-all duration-300 cursor-pointer">
                    ...
                    <div>
                      <h3 className="text-white font-semibold group-hover:text-white/90 transition-colors">{item.title}</h3>
                      <p className="text-white/50 text-sm group-hover:text-white/70 transition-colors">{item.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="lg:col-span-3 relative flex justify-center lg:justify-end">
              <div className="group relative w-full max-w-4xl">
                <div className="relative overflow-hidden rounded-3xl border border-white/20 shadow-2xl transform transition-all duration-700 group-hover:scale-105 group-hover:rotate-1">
                  <img src="/h3.jpeg" alt="Knowledge has a beginning but no end" className="w-full h-[600px] object-contain transition-all duration-700 group-hover:scale-110" />
